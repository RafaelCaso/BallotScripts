...
describe("Ballot", async () => {
  ...
  describe("when the chairperson interacts with the giveRightToVote function in the contract", async () => {
    ...
    it("can not give right to vote for someone that has voted", async () => {
      ...
      await ballotContract.write.giveRightToVote([otherAccount.account.address]);
      await contractAsOtherAccount.write.vote([0n]);
      expect(contractAsOtherAccount.write.vote([0n])).to.be.rejectedWith("The voter already voted.");

    });
    it("can not give right to vote for someone that has already voting rights", async () => {
      const {ballotContract, otherAccount} = await loadFixture(deployContract)
      await ballotContract.write.giveRightToVote([otherAccount.account.address])
      expect(ballotContract.write.giveRightToVote([otherAccount.account.address])).to.be.rejectedWith("has already voted");
    });
  });
  ...
  describe("when an account other than the chairperson interacts with the giveRightToVote function in the contract", async () => {
    ...
    it("should revert", async () => {
      const {ballotContract, otherAccount} = await loadFixture(deployContract);
      expect(ballotContract.write.giveRightToVote([otherAccount.account.address])).to.be.rejectedWith("Caller is not the chairperson");
    });
  });
  ...
  describe("when an account without right to vote interacts with the vote function in the contract", async () => {
    ...
    it("should revert", async () => {
      const {ballotContract} = await loadFixture(deployContract);
      expect(ballotContract.write.vote([0n])).to.be.rejectedWith("Has no right to vote");
    });
  });
